...
bool Core::getRandomOutputs(uint64_t amount, uint16_t count, std::vector<uint32_t>& globalIndexes,
  ...
  throwIfNotInitialized();
  logger(Logging::DEBUGGING) << "gRO counts: " << count;
  if (count == 0) {
    return true;
  }
  logger(Logging::DEBUGGING) << "mined money unlock window: " << currency.minedMoneyUnlockWindow();

  auto upperBlockLimit = getTopBlockIndex() - currency.minedMoneyUnlockWindow();
  logger(Logging::DEBUGGING) << "upperBlockLimit: " << upperBlockLimit;
