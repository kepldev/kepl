...
{
    uint64_t dollars = amount / (100 * 1000000);
    uint64_t cents = amount % (100 * 1000000);

    return formatDollars(dollars) + "." + formatCents(cents) + " KEPL";
}
...
std::string formatDollars(uint64_t amount)
{
    /* We want to format our number with comma separators so it's easier to
       ...
       such as ja_JP.utf8, 1 KEPL will actually be formatted as 100 KEPL, which
