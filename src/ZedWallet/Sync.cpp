                           ...
                           bool fetchTimestamp)
{
    ...
    if (fetchTimestamp)
    {
        ...
        /* Couldn't get timestamp, maybe old node or kepld closed */
        if (blockTime != "")
        {
...
void saveCSV(CryptoNote::WalletGreen &wallet, CryptoNote::INode &node)
{
    ...
    for (size_t i = 0; i < numTransactions; i++)
    {
        ...
        if (t.totalAmount != 0) {
            ...
            if (t.totalAmount < 0)
            {
                /* Put KEPL in separate field, makes output more usable in spreadsheet */
                std::string splitAmtKEPL = formatAmount(-t.totalAmount);
                boost::replace_all(splitAmtKEPL, " ", ",");
                myfile << "-" << splitAmtKEPL << ",OUT\n";
            }
            else
            {
                std::string splitAmtKEPL = formatAmount(t.totalAmount);
                boost::replace_all(splitAmtKEPL, " ", ",");
                myfile << splitAmtKEPL << ",IN\n";
            }
        }
    }
    myfile.close();
                ...
                std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    {
        std::cout << "Your kepld isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting kepld.\n"
                    "If this persists, visit "
                    "https://kepl.lol/#contact for support.";
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[KEPL " + shortName + "]: ");
}
